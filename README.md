...
### 교사용 - 회의실 생성 ⭐ v4.3.0 변경

1. **교사 이름 입력**: 예) "김선생님"
2. **회의실 비밀번호**: 기본값 `1234` (변경 가능)
3. **회의실 코드**: 기본값 `class01` (변경 가능)
   - 💡 기본값 그대로 사용하거나 원하는 코드로 변경
   - 예) "math2024", "class01", "room123" 등
4. **회의실 생성** 버튼 클릭
...
### 학생용 - 회의실 참여 ⭐ v4.3.0 변경

**방법1: 초대링크 사용** ⭐ 추천
1. 교사가 공유한 **초대링크** 클릭
2. 자동으로 회의실 참여 페이지 열림
3. **학생 이름 입력**: 예) "홍길동"
4. **회의실 비밀번호 입력** ⭐ NEW: 교사에게 받은 비밀번호
5. **회의실 참여** 버튼 클릭
6. 비밀번호 자동 검증
7. 카메라/마이크 권한 허용
8. 승인되면 자동으로 교사와 연결
...
- **🗑️ 전체삭제**: 화이트보드 전체 지우기
- **💾 PNG저장**: 화이트보드를 PNG 이미지로 저장 ⭐ NEW
- **📄 PDF저장**: 화이트보드를 PDF 문서로 저장 ⭐ NEW
